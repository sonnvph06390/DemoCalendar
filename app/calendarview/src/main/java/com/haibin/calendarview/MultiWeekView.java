...
public abstract class MultiWeekView extends BaseWeekView {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < 7; i++) {
            ...
            if (hasScheme) {
                boolean isDrawSelected = false;//是否继续绘制选中的onDrawScheme Có tiếp tục vẽ bản vẽ đã chọn trênDrawScheme không
                if (isSelected) {
                    isDrawSelected = onDrawSelected(canvas, calendar, x, true, isPreSelected, isNextSelected);
                }
                if (isDrawSelected || !isSelected) {
                    //将画笔设置为标记颜色  Đặt cọ thành màu đánh dấu
    ...
    /**
     * 日历是否被选中 Lịch được chọn
     *
     * @param calendar calendar
     * @return 日历是否被选中 Lịch được chọn
     */
    ...
    /**
     * 上一个日期是否选中 Cho dù ngày trước được chọn
     *
     * @param calendar 当前日期 Ngày hiện tại
     * @return 上一个日期是否选中 Cho dù ngày trước được chọn
     */
    ...
    /**
     * 下一个日期是否选中 Liệu ngày tiếp theo được chọn
     *
     * @param calendar 当前日期 Ngày hiện tại
     * @return 下一个日期是否选中 Liệu ngày tiếp theo được chọn
     */
     ...
     * @param isSelectedPre  上一个日期是否选中 Cho dù ngày trước được chọn
     * @param isSelectedNext 下一个日期是否选中 Liệu ngày tiếp theo được chọn
     * @return 是否绘制 onDrawScheme Có nên vẽ
     */
