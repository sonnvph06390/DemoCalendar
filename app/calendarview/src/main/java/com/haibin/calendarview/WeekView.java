 ...
 * Chế độ xem theo tuần, vì giao diện người dùng lịch được triển khai bằng cách cắm nóng,
...
public abstract class WeekView extends BaseWeekView {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mItems.size(); i++) {
            ...
            if (hasScheme) {
                boolean isDrawSelected = false;//是否继续绘制选中的onDrawScheme Có nên tiếp tục vẽ cái đã chọn
                if (isSelected) {
    ...
    public boolean onLongClick(View v) {
        ...
        if (mDelegate.isPreventLongPressedSelected()) {//如果启用拦截长按事件不选择日期 Nếu bạn bật chặn các sự kiện báo chí dài, không chọn ngày
            if (mDelegate.mCalendarLongClickListener != null) {
